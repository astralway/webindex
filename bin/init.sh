...
CC_DATA_JAR=$CC_HOME/modules/data/target/cc-data-0.0.1-SNAPSHOT.jar
if [ ! -f $CC_DATA_JAR ]; then
  echo "Failed to build $CC_DATA_JAR"
  exit 1
fi
CC_DATA_DEP_JAR=$CC_HOME/modules/data/target/cc-data-0.0.1-SNAPSHOT-jar-with-dependencies.jar
if [ ! -f $CC_DATA_DEP_JAR ]; then
  echo "Failed to build $CC_DATA_DEP_JAR"
  exit 1
fi
# If running OSX, remove file from Jar that causes problems
if [[ "$OSTYPE" == "darwin"* ]]; then
  zip -d $CC_DATA_DEP_JAR META-INF/LICENSE
fi
...
echo "io.fluo.observer.0=io.fluo.commoncrawl.data.fluo.PageObserver" >> $APP_PROPS
echo "io.fluo.observer.1=io.fluo.commoncrawl.data.fluo.InlinksObserver" >> $APP_PROPS
echo "io.fluo.observer.2=io.fluo.commoncrawl.data.fluo.IndexExporter" >> $APP_PROPS
java -cp $CC_DATA_DEP_JAR io.fluo.commoncrawl.data.PrintProps $CC_HOME/conf/data.yml >> $APP_PROPS
...
spark-submit --class io.fluo.commoncrawl.data.Init \
    ...
    $CC_DATA_DEP_JAR $CC_HOME/conf/data.yml
